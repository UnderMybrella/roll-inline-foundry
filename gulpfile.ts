...
function buildTS() {
    ...
    return res.js
        // .pipe(gulp_debug({title: "build_ts"}))
...
const bundleModule = () => {
    ...
    return bsfy.on('error', Logger.Err)
        .plugin(tsify)
        .bundle()
        // .pipe(gulp_debug({title: "bundle_module"}))
...
const buildLess = () => {
    return gulp.src("Source/Style/*.less")
        // .pipe(gulp_debug({title: "build_less"}))
        .pipe(less())
        .pipe(gulp.dest("dist"));
}
...
const gitTaskManifest = (cb: gulp.TaskFunctionCallback) => {
    ...
    return gulp.src([`package.json`, `Source/module.json`])
        // .pipe(gulp_debug({title: "git_task_manifest"}))
...
const gitTaskBuild = (cb: gulp.TaskFunctionCallback) => {
    ...
    return gulp.src(`dist/${manifest.file.name}-v${manifest.file.version}.zip`)
        // .pipe(gulp_debug({title: "git-task-build"}))
